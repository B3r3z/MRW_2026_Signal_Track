...
The system controller runs on a dedicated central PC connected to the same network as all other nodes. It fulfills two primary functions: (1)~coordinate the mobile transmitter units, including issuing motion commands and setting transmission parameters, and (2)~collect and process IQ~samples received from stationary receiver nodes.

Communication between the central PC and the mobile units follows a subscribe model published based on the MQTT protocol. Each mobile unit, upon startup, registers its hardware capabilities (e.g., supported frequency bands, available TX channels) with the controller and reports readiness after successful initialization of the SDR hardware and onboard processing pipelines. During runtime, the controller publishes experiment parameters such as carrier frequency, waveform type, transmission schedule, per-unit phase or amplitude offsets, and mobility commands to the individual transmitter units.
...
The core integration logic is located on the NVIDIA Jetson platform, which functions as the local computational node for both the transmitter (TX) and receiver (RX) units. This platform hosts the software modules that interface with the USRP hardware via the UHD driver, translating high-level system commands into specific waveform configurations and transmission timings.

Although motion control and radio operations are handled by distinct hardware components, the message handling architecture unifies them into a coherent system. The communication framework ensures that a single control stream can efficiently synchronize vehicle positioning with radio transmission, maintaining logical separation of duties while preserving system-wide consistency.

Connectivity with the controller of the central system is achieved through a publish–subscribe model based on the MQTT protocol. Upon startup, each mobile unit registers its capabilities and reports readiness only after successful initialization of both the SDR hardware and internal processing pipelines. During operation, the central controller issues experiment parameters, while the nodes return status updates or collected metrics.
